...
request == \* (id, amount, recipient, creation, initiator)
    REQUEST_IDS 
    \X MONEY 
    \X NETWORK_ADDRESSES 
...
Request(address2, id, amount, recipient) ==
    /\ amount > 0
    /\ address2 \in tier_two_addresses
    /\ requests' = requests \union <<id, amount, recipient, block_number, address2>>
...
Next ==
  ...
  \/ \E amount \in MONEY: 
        Deposit(amount)
  \/ \E <<address2, id, amount, recipient, creation>> 
    \in ADDRESSES \X REQUEST_IDS \X MONEY \X NETWORK_ADDRESSES:
        Request(address2, id, amount, recipient)
  \/ \E id \in REQUEST_IDS: 
        Withdraw(id) 
...
ADDRESSES_const == 0..2
NETWORK_ADDRESSES_const == 0..2
MONEY_const == 0..3
REQUEST_IDS_const == 0..5
